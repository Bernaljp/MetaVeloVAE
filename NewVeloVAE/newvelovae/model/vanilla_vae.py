...
import os
import time
from newvelovae.plotting import (plot_phase,
                              plot_sig,
                              plot_time,
                              plot_train_loss,
                              plot_test_loss)
...
class encoder_out(nn.Module):
    ...
    def __init__(self,
        ...
        self.eb_data1 = fw_encoder_block(Cin,N1).to(device)
        self.eb_mdata1 = fw_encoder_block(Cin,N1).to(device)
        self.eb_data2 = fw_encoder_block(2*N1,N2).to(device)
        self.eb_mdata2 = fw_encoder_block(N1,N2).to(device)

        self.fc_mu_z1 = nn.Linear(2*N2,dim_z1).to(device)
        self.fc_std_z1 = nn.Linear(2*N2,dim_z1).to(device)
        self.spz1 = nn.Softplus()

        if checkpoint is not None:
            self.load_state_dict(torch.load(checkpoint[0], map_location=device))
        else:
            self.init_weights()
...
class encoder_in(nn.Module):
    ...
    def __init__(self,
        ...
        self.fc_mu_t = nn.Linear(dim_z1, 1).to(device)
        self.spt1 = nn.Softplus()
        self.fc_std_t = nn.Linear(dim_z1, 1).to(device)
        self.spt2 = nn.Softplus()

        if checkpoint is not None:
            self.load_state_dict(torch.load(checkpoint[1], map_location=device))
        else:
            self.init_weights()

    def init_weights(self):
        for m in [self.fc_mu_t, self.fc_std_t]:
            nn.init.xavier_uniform_(m.weight)
            nn.init.constant_(m.bias, 0.0)
...
class VanillaVAE():
    def __init__(self,
        ...
        self.p_z1 = torch.stack([torch.zeros(adata.shape[0], dim_z1, device=self.device),
                                torch.ones(adata.shape[0], dim_z1, device=self.device)])\
                        .float()
    ...
    def vae_risk(self, q_z1, p_z1, q_tx, p_t, u, s, uhat, shat, sigma_u, sigma_s, weight=None, b=1.0, bz=1.0):
        # This is the negative ELBO.
        kldt = self.kl_time(q_tx[0], q_tx[1], p_t[0], p_t[1], tail=self.config["tail"])
        kldz1 = self.kl_z(q_z1[0], q_z1[1], p_z1[0], p_z1[1])
        ...
        return (- err_rec + b*(kldt) + bz*kldz1)
    ...
    def train_epoch(self, train_loader, test_set, optimizer, optimizer2=None, K=1):
        ...
        for i, batch in enumerate(train_loader):
            ...
            mu_tx, std_tx, t_global, uhat, shat, mu_z1, std_z1 = self.forward(xbatch, mbatch)

            loss = self.vae_risk((mu_z1, std_z1),
                                 self.p_z1[:, self.train_idx[idx], :],
                                 (mu_tx, std_tx),
    ...
    def train(self,
        ...
        train_set = SCData(X[self.train_idx], cell_labels_raw[self.train_idx], meta_X, meta_idx[self.train_idx])
        test_set = None
        if len(self.test_idx) > 0:
            test_set = SCData(X[self.test_idx], cell_labels_raw[self.test_idx], meta_X, meta_idx[self.test_idx])
        ...
        param_nn = list(self.encoder_out.parameters()) + list(self.encoder_in.parameters())
    ...
    def pred_all(self, data, meta_data, meta_idx, mode='test', output=["uhat", "shat", "t", "z1"], gene_idx=None):
        ...
        if "z1" in output:
            z1_out = np.zeros((N,self.config["dim_z1"]))
            std_z1_out = np.zeros((N,self.config["dim_z1"]))
        elbo = 0
        with torch.no_grad():
            B = min(N//10, 1000)
            Nb = N // B
            for i in range(Nb):
                ...
                mu_tx, std_tx, uhat, shat, mu_z1, std_z1 = self.eval_model(data_in, meta_in)
                if mode == "test":
                    p_t = self.p_t[:, self.test_idx[i*B:(i+1)*B], :]
                    p_z1 = self.p_z1[:, self.test_idx[i*B:(i+1)*B], :]
                elif mode == "train":
                    p_t = self.p_t[:, self.train_idx[i*B:(i+1)*B], :]
                    p_z1 = self.p_z1[:, self.train_idx[i*B:(i+1)*B], :]
                else:
                    p_t = self.p_t[:, i*B:(i+1)*B, :]
                    p_z1 = self.p_z1[:, i*B:(i+1)*B, :]
                loss = self.vae_risk((mu_z1, std_z1),
                                     p_z1,
                                     (mu_tx, std_tx),
                                     p_t,
                ...
                if "z1" in output:
                    z1_out[i*B:(i+1)*B] = mu_z1.detach().cpu().squeeze().numpy()
                    std_z1_out[i*B:(i+1)*B] = std_z1.detach().cpu().squeeze().numpy()
            if N > B*Nb:
                ...
                if mode == "test":
                    ...
                    p_z1 = self.p_z1[:, self.test_idx[B*Nb:], :]
                elif mode == "train":
                    p_t = self.p_t[:, self.train_idx[B*Nb:], :]
                    p_z1 = self.p_z1[:, self.train_idx[B*Nb:], :]
                else:
                    p_t = self.p_t[:, B*Nb:, :]
                    p_z1 = self.p_z1[:, B*Nb:, :]
                loss = self.vae_risk((mu_z1, std_z1),
                ...
                if "z1" in output:
                    z1_out[Nb*B:] = mu_z1.detach().cpu().squeeze().numpy()
                    std_z1_out[Nb*B:] = std_z1.detach().cpu().squeeze().numpy()
        out = []
        if "uhat" in output:
            out.append(Uhat)
        if "shat" in output:
    ...
    def test(self,
        ...
        self.set_mode('eval')
        data = test_set.data
        meta_data = test_set.metacells
        meta_idx = test_set.metacells_index
        mode = "test" if test_mode else "train"
        out, elbo = self.pred_all(data, meta_data, meta_idx, mode, gene_idx=gind)
    ...
    def save_model(self, file_path, enc_name='encoder_vanilla', dec_name='decoder_vanilla'):
        ...
        torch.save(self.encoder_out.state_dict(), f"{file_path}/{enc_name}_out.pt")
        torch.save(self.encoder_in.state_dict(), f"{file_path}/{enc_name}_in.pt")

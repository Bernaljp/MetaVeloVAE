...
import time
from newvelovae.plotting import plot_sig, plot_time, plot_vel
from newvelovae.plotting import plot_train_loss, plot_test_loss
...
class fw_encoder_block(nn.Module):
    def __init__(self,Cin,Cout):
        super(fw_encoder_block,self).__init__()
...
class encoder_out(nn.Module):
    ...
    def __init__(self,
        ...
        self.eb_data1 = fw_encoder_block(Cin,N1)
        self.eb_mdata1 = fw_encoder_block(Cin,N1)
        self.eb_data2 = fw_encoder_block(2*N1,N2)
        self.eb_mdata2 = fw_encoder_block(N1,N2)

        self.fc_mu_z1 = nn.Linear(2*N2+dim_cond,dim_z1)
        self.fc_std_z1 = nn.Linear(2*N2+dim_cond,dim_z1)
        self.spz1 = nn.Softplus()
    ...
    def init_weights(self):
        ...
        for m in [self.eb_data1,self.eb_mdata1,self.eb_data2,self.eb_mdata2]:
            m.init_weights()
        
        for m in [self.fc_mu_z1, self.fc_std_z1]:
            nn.init.xavier_uniform_(m.weight)
            nn.init.constant_(m.bias, 0.0)

    def forward(self, data_in, meta_data_in, condition=None):
        h = self.eb_data1(data_in)
        mh = self.eb_mdata1(meta_data_in)
        h = torch.cat((h,mh), 1)
        h = self.eb_data2(h)
        mh = self.eb_mdata2(mh)
        h = torch.cat((h,mh), 1)

        if condition is not None:
            h = torch.cat((h, condition), 1)
        
        mu_zx, std_zx = self.fc_mu_z1(h), self.spz1(self.fc_std_z1(h))
        return mu_zx, std_zx
...
class encoder_in(nn.Module):
    def __init__(self,
        ...
        self.fc_mu_t = nn.Linear(dim_z1, 1)
        self.spt1 = nn.Softplus()
        self.fc_std_t = nn.Linear(dim_z1, 1)
        self.spt2 = nn.Softplus()
        self.fc_mu_z = nn.Linear(dim_z1, dim_z)
        self.fc_std_z = nn.Linear(dim_z1, dim_z)
    ...
    def forward(self, z1):
        ...
        mu_tx, std_tx = self.spt1(self.fc_mu_t(z1)), self.spt2(self.fc_std_t(z1))
        mu_zx, std_zx = self.fc_mu_z(z1), self.spt3(self.fc_std_z(z1))
...
class VAE(VanillaVAE):
    ...
    def __init__(self,
        ...
        try:
            G = adata.n_vars
            self.encoder_out = encoder_out(2*G,
                                   dim_z1,
                                   dim_cond,
                                   hidden_size[0],
                                   hidden_size[1]).float().to(self.device)
            self.encoder_in = encoder_in(dim_z1,
                                         dim_z).float().to(self.device)
        except IndexError:
            print('Please provide two dimensions!')
        if checkpoints[0] is not None:
            self.encoder_out.load_state_dict(torch.load(checkpoints[0], map_location=device))
            self.encoder_in.load_state_dict(torch.load(checkpoints[0], map_location=device))
        ...
        self.p_z1 = torch.stack([torch.zeros(adata.shape[0], dim_z1, device=self.device),
                                torch.ones(adata.shape[0], dim_z1, device=self.device)*self.config["std_z1_prior"]])\
                        .float()
    ...
    def forward(self,
        ...
        mu_z1, std_z1 = self.encoder_out.forward(data_in_scale, adata_in)
        z1 = self.sample(mu_z1, std_z1)

        mu_t, std_t, mu_z, std_z = self.encoder_in.forward(z1)
        ...
        return mu_t, std_t, mu_z, std_z, t, z, uhat, shat, uhat_fw, shat_fw, vu, vs, vu_fw, vs_fw, mu_z1, std_z1
    ...
    def eval_model(self,
        ...
        if self.config["scale_gene_encoder"]:
            data_in_scale = torch.cat((data_in_scale[:, :data_in_scale.shape[1]//2]/self.decoder.scaling.exp(),
                                       data_in_scale[:, data_in_scale.shape[1]//2:]), 1)
        mu_z1, std_z1 = self.encoder_out.forward(data_in_scale, meta_in, condition)
        mu_t, std_t, mu_z, std_z = self.encoder_in.forward(mu_z1)
        ...
        return mu_t, std_t, mu_z, std_z, uhat, shat, uhat_fw, shat_fw, vu, vs, vu_fw, vs_fw, mu_z1, std_z1
    ...
    def _compute_kl_term(self, q_z1, p_z1, q_tx, p_t, q_zx, p_z):
        ##############################################################
        ...
        ##############################################################
        kldz1 = kl_gaussian(q_z1[0], q_z1[1], p_z1[0], p_z1[1])
        ...
        return (self.config["kl_z1"]*kldz1
                + self.config["kl_t"]*kldt
    ...
    def vae_risk_gaussian(self,
        ...
        """
        kl_term = self._compute_kl_term(q_z1, p_z1, q_tx, p_t, q_zx, p_z)
    ...
    def train_epoch(self, train_loader, test_set, optimizer, optimizer2=None, K=1):
        ...
        for i, batch in enumerate(train_loader):
            ...
            xbatch, idx, mbatch = batch[0].float().to(self.device), batch[3], batch[4].float().to(self.device).squeeze()
            ...
            (mu_tx, std_tx,
             ...
             vu, vs,
             vu_fw, vs_fw,
             mu_z1, std_z1) = self.forward(xbatch, mbatch, u0, s0, t0, t1, condition)
            if uhat.ndim == 3:
                lu_scale = lu_scale.unsqueeze(-1)
                ls_scale = ls_scale.unsqueeze(-1)
            loss = self.vae_risk((mu_z1, std_z1), self.p_z1[:, self.train_idx[idx], :],
            ...
            loss.backward()
            # gradient clipping
            torch.nn.utils.clip_grad_value_(self.encoder_out.parameters(), GRAD_MAX)
            torch.nn.utils.clip_grad_value_(self.encoder_in.parameters(), GRAD_MAX)
    ...
    def update_x0(self, U, S, mU, mS, midx):
        ##########################################################################
    ...
    def train(self,
        ...
        if self.is_discrete:
            ...
            meta_U,meta_S = meta_adata.layers['unspliced'], meta_adata.layers['spliced']
        else:
            X = np.concatenate((adata.layers['Mu'], adata.layers['Ms']), 1).astype(float)
            meta_U,meta_S = meta_adata.layers['Mu'], meta_adata.layers['Ms']
        try:
        ...
        meta_X = np.concatenate((meta_U,meta_S), 1).astype(float).squeeze()
        meta_idx = adata.obs['SEACell'].str.extract(r'(\d+)').astype(int).values
        ...
        train_set = SCData(X[self.train_idx], self.cell_labels[self.train_idx], meta_X, meta_idx[self.train_idx])
        test_set = None
        if len(self.test_idx) > 0:
            test_set = SCData(X[self.test_idx], self.cell_labels[self.test_idx], meta_X, meta_idx[self.test_idx])
        ...
        param_nn = list(self.encoder_out.parameters())\
            + list(self.encoder_in.parameters())\
        ...
        print("*********                      Stage  2                       *********")
        self.encoder_out.eval()
        self.encoder_in.eval()
        self.use_knn = True
        self.train_stage = 2
        ...
        for r in range(self.config['n_refine']):
            ...
            if (not self.is_discrete) and (noise_change > 0.001) and (r < self.config['n_refine']-1):
                self.update_std_noise(train_set.data, train_set.metacells, train_set.metacells_index)
                stop_training = False
            if stop_training:
                print(f"Stage 2: Early Stop Triggered at round {r}.")
                break
            self.update_x0(X[:, :X.shape[1]//2], X[:, X.shape[1]//2:], meta_U, meta_S, meta_idx)
            self.n_drop = 0
    ...
    def pred_all(self, data, meta_data, meta_idx, cell_labels, mode='test', output=["uhat", "shat", "z1", "t", "z"], gene_idx=None):
        ...
        if "z1" in output:
            z1_out = np.zeros((N, self.dim_z1))
            std_z1_out = np.zeros((N, self.dim_z1))
        if "t" in output:
            t_out = np.zeros((N))
        ...
        with torch.no_grad():
            ...
            for i in range(Nb):
                ...
                meta_in = torch.tensor(meta_data[meta_idx[i*B:(i+1)*B]], device=self.device).float().squeeze()
                if mode == "test":
                ...
                (mu_tx, std_tx,
                 ...
                 vu, vs,
                 vu_fw, vs_fw,
                 mu_z1, std_z1) = self.eval_model(data_in, meta_in, u0, s0, t0, t1, y_onehot)
                ...
                loss = self.vae_risk((mu_z1, std_z1), p_z1,
                                     (mu_tx, std_tx), p_t,
                                     (mu_zx, std_zx), p_z,
                                     data_in[:, :G], data_in[:, G:],
                                     uhat*lu_scale, shat*ls_scale,
                ...
                if "z1" in output:
                    z1_out[i*B:(i+1)*B] = mu_z1.detach().cpu().numpy()
                    std_z1_out[i*B:(i+1)*B] = std_z1.detach().cpu().numpy()
                if "t" in output:
            ...
            if N > B*Nb:
                ...
                meta_in = torch.tensor(meta_data[meta_idx[Nb*B:]], device=self.device).float().squeeze()
                meta_in = meta_in if len(meta_in.shape)>1 else meta_in[None,:]
                if mode == "test":
                ...
                if "z1" in output:
                    z1_out[Nb*B:] = mu_z1.detach().cpu().numpy()
                    std_z1_out[Nb*B:] = std_z1.detach().cpu().numpy()
                if "t" in output:
    ...
    def test(self,
        ...
        if self.train_stage == 2:
            out_type.append("v")
        out, elbo = self.pred_all(dataset.data, dataset.metacells, dataset.metacells_index, self.cell_labels, mode, out_type, gind)
    ...
    def update_std_noise(self, train_data, train_meta_adata, train_meta_idx):
    ...
    def save_anndata(self, adata, meta_adata, key, file_path, file_name=None):
        ...
        if self.is_discrete:
            U, S = np.array(adata.layers['unspliced'].todense()), np.array(adata.layers['spliced'].todense())
            meta_X = np.concatenate((meta_adata.layers['unspliced'], meta_adata.layers['spliced']),axis=1).squeeze()
        else:
            U, S = adata.layers['Mu'], adata.layers['Ms']
            np.concatenate((meta_adata.layers['Mu'], meta_adata.layers['Ms']),axis=1).squeeze()
            meta_X = np.concatenate((meta_adata.layers['Mu'], meta_adata.layers['Ms']),axis=1).squeeze()
        ...
        Uhat, Shat, z1, std_z1, t, std_t, z, std_z = out["uhat"], out["shat"], out["z1"], out["std_z1"], out["t"], out["std_t"], out["z"], out["std_z"]

        adata.obsm[f"{key}_z1"] = z1
        adata.obsm[f"{key}_std_z1"] = std_z1
